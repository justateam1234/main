...
3.    Whether these small fishing companies should change the way they operate, what is the best way to run a small fishing compan
...
$r$       & Capacity factor                                                          \\ \hline
 ...
 \subsection{The fish-temperature model}
    我们知道，鱼群的都需要适合的栖息环境以生存，若栖息环境改变，鱼群便会选择迁移。在我们之前的数据和预测中，我们可以看到全球变暖使得海面温度的变化十分可观，而温度又是在鱼群的繁衍、捕食环境有着关键影响的因素。因此我们将来自***的渔业数据和温度数据做了个对比。
    \includegraphics[width = .6\textwidth]{fish-temp1.eps}
    我们通过渔业数据中在某地点的捕捞量来代指此地点的鱼群密度，将抽样地点的温度和鱼群密度进行对比（如下图）。
...
\begin{figure}[tbp]
  \centering{
  \includegraphics[width=0.6\textwidth]{./picture/figure.jpeg}}
